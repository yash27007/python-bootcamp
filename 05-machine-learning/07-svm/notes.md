...
**Hypothesis (stated before running):** as $C$ increases from small to large on a fixed dataset, the margin width should *shrink*, the number of support vectors should *decrease* (fewer points are allowed to sit inside a shrinking margin, and any point strictly outside the margin isn't a support vector), and training accuracy should *increase* (more tolerance for outliers removed) — reproducing the same bias-variance U-shape from `05b-bias-variance-tradeoff` (referenced via `04-regularization`), just with $C$ playing the role of an inverse complexity/regularization knob instead of $\lambda$ or polynomial degree.
